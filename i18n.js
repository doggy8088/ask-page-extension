(() => {
    ...
    function translateText(value) {
        ...
        const translatedValue = normalizedValue
            ...
            .replace(/^(⏳\s*)?(\[\d+\/\d+\]\s*)?正在分析需求與頁面狀態。$/, '$1$2Analyzing the request and page state.')
            .replace(/^(⏳\s*)?(\[\d+\/\d+\]\s*)?這個端點不支援 tool calling，我改用一般文字模式繼續。$/, '$1$2This endpoint does not support tool calling. Continuing in plain-text mode.')
            .replace(/^(⏳\s*)?(\[\d+\/\d+\]\s*)?這次回應在輸出上限前就被截斷了，我會放寬輸出額度再試一次。$/, '$1$2The response was truncated before reaching the output limit. Retrying with a higher output limit.')
            .replace(/^(⏳\s*)?(\[\d+\/\d+\]\s*)?這次沒有拿到可顯示內容，我再試一次。$/, '$1$2No displayable content was received. Retrying.')
            .replace(/^(⏳\s*)?(\[\d+\/\d+\]\s*)?服務暫時不穩定，我會稍候自動重試。$/, '$1$2The service is temporarily unstable. Retrying shortly.')
