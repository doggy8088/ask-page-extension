...
- 為每個命令設定專屬的提示內容，並在提示內容中以 `${變數名}` 產生填空，讓使用者執行命令後逐一填寫
- 編輯或刪除已建立的自訂命令
- 所有自訂命令會自動出現在智慧提示中

#### 提示內容的變數填空

在提示內容中以 `${變數名}` 即可產生填空欄位，使用者輸入命令後按 Tab 展開，游標會停在第一個變數上，填寫後按 Tab 跳到下一個變數，全部填完按 Enter 送出。例如提示內容 `Hi, I'm ${name} from ${country:TW}`，執行時會展開成可填寫的範本，`${country:TW}` 的 `TW` 會作為預設值並被選取，可直接覆寫或保留。
...
- 編輯命令時可勾選「展開提示詞時顯示變數名稱」：預設不勾選時，尚未填寫的變數只會以變數名稱本身作為暫時提示顯示，一旦輸入內容提示就會消失；勾選後則維持「變數名稱：」的標籤持續顯示。此設定同時適用於內建的 `/summary` 提示編輯。
...
4. 輸入提示內容（當使用此命令時要傳送給 AI 的指令；可在提示中以 `${變數名}` 產生填空，例如 `Hi, I'm ${name} from ${country:TW}`）
5. 點擊「儲存」完成新增
6. 已建立的命令可以隨時編輯或刪除
