...
使用 Gemini、OpenAI、Azure OpenAI、Anthropic、DeepSeek、OpenRouter、Groq、Mistral、Meta AI、Ollama 或 OpenAI Compatible API 詢問關於目前頁面的問題。這是一個 Chrome 擴充功能，讓您可以快速與頁面內容互動，支援多種 AI 提供者。

## ✨ 功能特色

- 🤖 **多 AI 提供者支援** - 整合 Google Gemini、OpenAI、Azure OpenAI、Anthropic、DeepSeek、OpenRouter、Groq、Mistral、Meta AI、Ollama Local、Ollama Cloud 與 OpenAI Compatible，可自由切換
- **Gemini Google Search Grounding** - Gemini 提供者設定可選啟用 Google Search Grounding（預設關閉）；啟用後詢問模式與 Gemini 3 代理模式可使用網路搜尋（代理模式可同時使用頁面工具），勾選時會顯示免費方案不支援的警示
...
- 🟠 **代理模式網頁背光提示** - 代理模式從送出問題到回覆完成期間，網頁四周會顯示琥珀色背光動畫，讓您一眼就能看出 AskPage 正在頁面上工作；可在設定頁「一般設定」分頁關閉，並遵循系統減少動態效果偏好
...
啟用 `/screenshot` 後，每次送出一般提問、`/summary` 或自訂技能時，AskPage 都會先隱藏對話框並擷取目前可視範圍，再把同一張截圖附加到使用者訊息右上角。滑鼠移到縮圖上可放大預覽，點擊縮圖可在新視窗檢視完整圖片。
...
- 新增技能（例如：`/翻譯`、`/解釋`、`/重點整理`）
- 為每個技能設定專屬的提示內容，並在提示內容中以 `${變數名}` 產生填空，讓使用者執行技能後逐一填寫
- 編輯或刪除已建立的技能
- 所有技能會自動出現在技能快速提示中

#### 提示內容的變數填空

在提示內容中以 `${變數名}` 即可產生填空欄位，使用者輸入技能名稱後按 Tab 展開，游標會停在第一個變數上，填寫後按 Tab 跳到下一個變數，全部填完按 Enter 送出。例如提示內容 `Hi, I'm ${name} from ${country:TW}`，執行時會展開成可填寫的範本，`${country:TW}` 的 `TW` 會作為預設值並被選取，可直接覆寫或保留。

- `${變數名}`：產生填空欄位，無預設值。
- `${變數名:預設值}`：產生填空欄位並帶預設值，預設值會被選取供直接覆寫。
- 同名變數會同步填寫，例如 `${name}` 出現多次時，改一個全部一起改。
- 變數名稱會先進行 Unicode NFC 正規化，因此 `é` 與由 `e` 加組合重音構成的等價形式會視為同一個變數。
- 同名變數最多只能有一組預設值，若兩個同名變數設了不同的預設值，儲存時會擋下。
- 變數內容以第一個 `}` 關閉，其餘的 `}` 視為字面文字；變數名與預設值以第一個 `:` 分隔，其餘的 `:` 屬於預設值的一部分。
- 編輯技能時可勾選「展開提示詞時顯示變數名稱」：預設不勾選時，尚未填寫的變數只會以變數名稱本身作為暫時提示顯示，一旦輸入內容提示就會消失；勾選後則維持「變數名稱：」的標籤持續顯示。此設定同時適用於內建的 `/summary` 提示編輯。

#### 內建變數 `${SELECTED_TEXT}`

`${SELECTED_TEXT}` 是系統保留變數，執行技能時會自動替換成開啟對話框當下在頁面選取的文字。它不是填空欄位：技能提示詞只含 `${SELECTED_TEXT}` 時，輸入技能名稱後按 Enter 或 Tab 會直接送出替換後的提示詞，不會進入填空流程。例如提示內容 `請解釋 ${SELECTED_TEXT} 這個名詞`，選取 `Hyrum's Law` 後執行，實際送出的是 `請解釋 Hyrum's Law 這個名詞`。

- `${SELECTED_TEXT}`：替換成選取文字。沒有選取文字時，技能會被擋下並顯示提示，請先選取文字並重新開啟對話框。
- `${SELECTED_TEXT:備用文字}`：沒有選取文字時改用備用文字送出，不會被擋下。
- 可以和一般填空變數混用，例如 `把 ${SELECTED_TEXT} 翻譯成 ${lang:英文}`：按 Tab 展開後只有 `lang` 需要填寫，選取文字會直接內嵌在提示詞中。
- 選取文字含有 `${...}` 字樣時會原文送出，不會被當成變數解析。
- 變數名稱區分大小寫，`${selected_text}` 仍是一般填空變數。
- 自訂的 `/summary` 提示詞同樣適用。
- 選取文字仍會照原本方式附在頁面上下文中提供給模型；此變數只是額外讓提示詞可以直接引用它。
...
1. 在設定頁面切換到「技能」頁籤
2. 點擊「新增技能」
3. 輸入技能名稱（必須以 `/` 開頭，例如：`/翻譯`）
4. 輸入提示內容（執行此技能時要傳送給 AI 的提示詞；可在提示中以 `${變數名}` 產生填空，例如 `Hi, I'm ${name} from ${country:TW}`）
5. 點擊「儲存」完成新增
6. 已建立的技能可以隨時編輯或刪除
