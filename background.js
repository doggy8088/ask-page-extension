...
async function executeMainWorldJavaScript(tabId, code) {
    ...
    function buildMainWorldExecutionScript(sourceCode) {
        return [
            ...
            '    }',
            '',
            '    // askpage.ref(id) / askpage.refs(id)：以 content script 在執行前暫時標上的 data-askpage-ref 屬性取得快照元素。',
            '    // 限制：querySelector 不會穿透 shadow DOM，位於 shadow root 內的元素需改以其他方式定位。',
            '    function normalizeAskPageRefId(value) {',
            '        const match = String(value ?? \'\').trim().toLowerCase().match(/^#?e?(\\d+)$/);',
            '        return match ? `e${Number(match[1])}` : \'\';',
            '    }',
            '',
            '    const askpage = Object.freeze({',
            '        ref(id) {',
            '            const normalizedId = normalizeAskPageRefId(id);',
            '            return normalizedId ? document.querySelector(`[data-askpage-ref="${normalizedId}"]`) : null;',
            '        },',
            '        refs(id) {',
            '            const normalizedId = normalizeAskPageRefId(id);',
            '            return normalizedId ? Array.from(document.querySelectorAll(`[data-askpage-ref="${normalizedId}"]`)) : [];',
            '        }',
            '    });',
            '',
            '    try {',
            '        const rawResult = await (async (window, document, selection, buildElementSelector, askpage) => {',
            sourceCode,
            '        })(window, document, getSelectionSnapshot(), buildElementSelector, askpage);',
            '',
            '        return {',
            '            success: true,',
