...
- **🎯 自訂斜線命令系統**: 全新的使用者自訂斜線命令功能
  - 使用者可以自由新增、編輯、刪除自訂斜線命令
  - 每個自訂命令包含命令名稱、斜線命令、描述和自訂提示語
  - 支援動態載入和執行自訂命令
  - 自訂命令會出現在智慧提示系統中
  - 命令驗證：確保命令格式正確且不與內建命令衝突
- **🎨 全新設定頁面介面**: 完全重新設計的完整頁面設定介面
  - 從小彈出視窗改為全頁面設定介面，大幅提升操作便利性
  - 三個主要頁籤：AI 提供者、內建命令、自訂命令
  - 現代化的視覺設計，支援深色/淺色主題自動切換
  - 響應式設計，支援各種螢幕尺寸
  - 流暢的頁籤切換動畫和使用者互動回饋
- **📥📤 設定匯入匯出功能**: 新增設定的備份和還原功能
  - 匯出設定：將自訂命令和其他設定匯出為 JSON 檔案
  - 匯入設定：從 JSON 檔案快速還原設定（為了安全，不包含 API Key）
  - 支援跨裝置設定同步和備份
- **🔧 自訂命令管理介面**: 完整的 CRUD 操作介面
  - 直觀的表單介面新增自訂命令
  - 編輯現有命令的完整功能
  - 一鍵刪除命令（含確認對話框）
  - 命令預覽和詳細資訊展示
...
- **🚀 彈出視窗優化**: 原有的設定彈出視窗改為快速連結至完整設定頁面
- **🧠 智慧命令提示增強**: intellisense 系統現在包含所有自訂命令
- **💾 向後相容性**: 完全保持與現有設定和功能的相容性
- **📝 歡迎訊息動態化**: 對話框歡迎訊息會動態顯示可用的自訂命令
- **🎯 命令執行優化**: 統一的命令處理系統，支援內建和自訂命令
- **🔍 設定管理增強**: 更強大的設定重置功能，保護重要資料

### 技術細節

- 新增 `CUSTOM_COMMANDS_STORAGE` 儲存鍵值管理自訂命令
- 實作完整的自訂命令資料結構：`{ id, name, command, description, prompt, enabled }`
- 新增 `options.html` 和 `options.js` 作為主要設定介面
- 修改 `popup.html` 作為設定頁面的快速入口
- 在 `manifest.json` 中新增 `options_page` 配置
- 更新 `content.js` 的命令處理邏輯以支援動態自訂命令
- 實作命令驗證系統，防止與內建命令衝突
- 新增設定匯入匯出的安全機制（排除敏感資料）

### 破壞性變更

- **主要版本升級**: 從 0.5.0 升級到 1.0.0（如要求的 major version bump）
- **介面變更**: 設定介面從彈出視窗改為完整頁面（提升使用體驗）
- 所有現有功能保持完全相容，無實際破壞性變更
...
<!-- 版本鏈接 -->
[1.0.0]: https://github.com/doggy8088/ask-page-extension/compare/v0.5.0...v1.0.0
