...
- **嚴格 CSP 頁面改經背景服務工作者載入語系檔**：在 Azure 入口與 Azure Web Apps 等設有嚴格 Content-Security-Policy 的頁面，內容腳本直接 `fetch` 擴充功能內部 `_locales` 檔案會被頁面 `connect-src` 政策攔截並拋出 `TypeError: Failed to fetch`。現改為直接讀取失敗時自動經 `get-locale-catalog` 訊息請背景服務工作者讀取，該路徑不受頁面 CSP 限制；直接讀取失敗只記為 `debug`，兩條路徑皆失敗才輸出原有警告。嚴格 CSP 頁面不再出現語系檔載入失敗警告，一般頁面仍優先直接讀取。
- **避免 KaTeX 在怪異模式頁面輸出警告與中斷公式渲染**：瀏覽缺少標準 doctype 的怪異模式頁面時，KaTeX 頂層的 `document.compatMode` 檢查會輸出主控台警告並將渲染函式替換為拋出例外的版本。由於第三方網站的 doctype 非擴充功能所能控制，且對話視窗有獨立容器與樣式隔離，已移除該檢查，怪異模式頁面仍可正常渲染 LaTeX 公式。
...
- **系統提示詞加入工具使用階梯**：由便宜到昂貴引導模型先用快照與 `find`，再用 ref 動作，最後才取 HTML；並說明 ref 的有效期與失效處理。

### 修正 / 更新（v0.45.0）

- **Gemini 工具回傳未截斷**：`functionResponse` 現在與 OpenAI 路徑一致，超過 6000 字元時改包成 `{ truncated, preview, note }`，避免 `run_js` 回傳整段 HTML 灌爆上下文。
- **`docs/TOOLS.md` 補上 `get_page_metadata` 與 Anthropic 無工具支援的說明**。

## [0.44.10] - 2026-09-07

### 新增 / 改進（v0.44.10）

- **技能提示詞新增內建變數 `${SELECTED_TEXT}`**：執行技能時自動替換成開啟對話框當下選取的文字，不會變成填空欄位；支援 `${SELECTED_TEXT:備用文字}` 在沒有選取時改用備用文字，沒有選取又沒有備用文字時擋下並提示先選取文字。可與一般 `${變數}` 混用，自訂 `/summary` 提示詞同樣適用；技能編輯視窗補上說明文字，五種語系同步更新。

## [0.44.9] - 2026-09-03

### 新增 / 改進（v0.44.9）

- **新增 muse-spark-1.3 系列模型至 Meta AI 提供者清單**：Meta AI（`meta`）提供者預設模型清單新增 `muse-spark-1.3-contributor` 與 `muse-spark-1.3`，並置於選項最前方，維持最新模型優先顯示之排序原則；完整支援 SSE 串流生成與 32768 最大輸出 Token，並同步更新 README 與單元測試。

## [0.44.8] - 2026-09-03

### 新增 / 改進（v0.44.8）

- **新增 Meta AI 提供者與 Muse Spark 系列模型支援**：新增 Meta AI（`meta`）為獨立提供者選項，預設內建 `muse-spark-1.2-contributor`、`muse-spark-1.2` 與 `muse-spark-1.1` 模型；完整支援 SSE 串流生成（最大輸出 32768 Token）、API Key 連線測試與動態載入模型清單（`https://api.meta.ai/v1/models`）。於偏好設定彈窗提供專屬樣式與表單驗證，並同步更新繁中、英文、簡中、日文與韓文 5 國語系文字、單元測試與 README 支援清單。

## [0.44.7] - 2026-09-02

### 新增 / 改進（v0.44.7）

- **新增 gemini-3.8-flash 模型**：Gemini 提供者預設模型清單新增 `gemini-3.8-flash`，並置於選項最前方，維持最新模型優先顯示之排序原則；完整支援串流回應、等級型推理強度控制（`minimal`／`low`／`medium`／`high`，預設 `medium`）與 65536 最大輸出 Token，並同步更新 README 與目前推理強度對應表文件。
...
- **找不到技能時改以一般提示詞送出**：在主要對話框輸入以斜線開頭、但未對應任何自訂技能的文字時，不再顯示「找不到技能」錯誤並中止流程，而是將原始輸入直接當成一般提示詞送出，交由模型自行判斷語意；該輸入同樣會納入提示詞歷史，並依現有設定附帶選取文字、頁面內容與網頁截圖。內建技能 `/clear`、`/summary`、`/screenshot`、`/agent` 的判斷仍在自訂技能比對之前完成，行為不變；含 `${變數}` 的技能範本也維持原有「請按 Tab 展開並填寫變數」的提示。

## [0.44.2] - 2026-08-25

### 新增 / 改進（v0.44.2）

- **偏好設定頁籤導向**：從使用提示中的「自訂技能 (N)」數字按鈕開啟偏好設定時，會直接切換至「技能管理」頁籤；偏好設定頁已開啟時也會即時切換，其他入口仍依原有設定還原上次使用的頁籤。

## [0.44.1] - 2026-08-24

### 修正 / 更新（v0.44.1）

- **關閉主要對話框不中止背景問答**：點擊對話框外部、按下 Escape 或透過切換指令關閉主要對話框時，進行中的提問與代理工作會繼續在背景執行；只有在重新開啟主要對話框後點擊紅色 STOP 按鈕，才會主動取消工作，並在重新開啟時恢復執行中的停止狀態。

## [0.44.0] - 2026-08-24

### 新增 / 改進（v0.44.0）

- **全模式提問任務終止與停止按鈕**：新增統一的提問任務生命週期管理，使用者送出提問後，原送出按鈕會切換為停止按鈕（48x48 紅底白方塊樣式），可隨時中斷一般詢問模式與代理模式的所有進行中工作。取消訊號會即時傳遞至服務工作者請求、串流讀取、重試等待、網頁截圖與各類工具執行（包括 Web Search、JavaScript 等），立即解除載入狀態並安全恢復輸入介面。
- **多語系停止狀態與無障礙優化**：停止按鈕同步提供正體中文、英文、簡體中文、日文與韓文的語系文字，並在對話框開啟、關閉、語系切換及任務完成或失敗時，即時更新 `aria-label`、`title` 與 `aria-busy` 屬性。
- **Ollama Cloud 預設模型清單與排序更新**：更新 Ollama Cloud 提供者之預設模型選項，將 `deepseek-v4-flash:0731-cloud` 改為 `deepseek-v4-flash:0731`、移除 `deepseek-v4-flash`、將 `deepseek-v4-pro` 改為 `deepseek-v4-pro:0813`，並新增 `gemma4:31b` 與 `kimi-k3`；同時依最新推薦順序重新排列模型清單。
...
- **執行時提供者顯示格式優化**：將分析頁面、任務規劃與錯誤狀態提示中的提供者顯示名稱調整為「`提供者自訂名稱 (模型名稱)`」（例如「`Google (gemini-3.7-flash)`」）。當使用者未自訂提供者名稱時，回退至語系化提供者類型標籤（例如「`Google Gemini (gemini-3.7-flash)`」），讓多模型切換與執行狀態更加一目了然。
- **Gemini Google Search Grounding 警示文案更新**：更新多語系（正體中文、簡體中文、英文、日文、韓文）與設定頁中的 Google Search Grounding 行內警示文案，明確標註「僅 Tier 1 以上付費方案可使用此功能」，避免使用者因方案層級混淆而誤啟用觸發 429 錯誤。
...
- **表單自動填寫工具優化**：調整 `fill_form_fields` 工具在 LLM 端的描述與參數說明，大幅提升模型在規劃填表任務時的調用傾向與精準度。
- **高擬真使用者輸入事件模擬**：重構事件派發機制，加入原生的 `focus()` 與 `blur()` 動作，並使用 `InputEvent` 派發 `insertText` 模擬鍵盤輸入，解決現代前端框架（如 React/Vue）在自動填表時無法偵測內部狀態變更的問題。
- **表單填寫嚴格校驗與引導提示**：將表單自動填寫成功判定調整為全數成功才回傳 true。若任一欄位填寫失敗，工具將回傳失敗，詳細條列各欄位錯誤原因，並提供明確的 `[重新規劃提示]`（引導 AI 在面臨複雜組件失敗時，主動切換至使用 `run_js` 工具撰寫自訂 JS 強制寫入與點選）。

## [0.42.1] - 2026-08-07

### 新增 / 改進（v0.42.1）

- **代理模式網頁背光提示效果**：代理模式從送出問題到回覆完成的整段請求期間（涵蓋所有工具呼叫回合與串流），會在目前網頁四周顯示琥珀色背光動畫，仿照 Perplexity Comet「Computer Use」執行時的邊緣提示，讓使用者一眼就能看出 AskPage 正在頁面上工作；背光完全不攔截滑鼠事件，也不影響頁面或對話框互動。
- **一般設定分頁與背光開關**：設定頁新增「一般設定」分頁，提供可關閉網頁背光效果的開關（預設開啟），並依系統 `prefers-reduced-motion` 偏好停用脈動動畫、僅保留靜態背光。

## [0.42.0] - 2026-08-06

### 新增 / 改進（v0.42.0）

- **完整多語系介面與系統提示**：將 AskPage 的介面文字、內建提示與 AI 預設回答語言統一納入執行期 i18n，支援正體中文、英文、簡體中文、日文與韓文；語系偏好可儲存並即時套用，缺少訊息時會安全後援至正體中文，同時保留使用者輸入、AI 回覆、網頁內容、自訂技能與程式碼區塊原文。
- **偏好語言回覆引導**：系統提示會明確要求模型優先使用 AskPage 設定的語言回答，只有使用者在當次提問明確指定其他語言時才切換；英文介面的截圖與代理狀態標題同步縮短為 `Screenshot: Off` 與 `Agent: On`。
- **技能術語統一**：設定頁、對話框提示、驗證訊息、README 與商店文案統一使用技能相關用語；保留既有斜線觸發語法、儲存鍵、DOM ID、CSS 類別、匯入匯出格式與使用者資料相容性，並將設定頁頁籤明確命名為技能管理。

### 修正 / 更新（v0.42.0）

- **設定匯入後重新整理來源網頁**：匯入 JSON 設定成功後，僅重新整理開啟設定頁前記錄的 HTTP／HTTPS 來源分頁，讓內容腳本重新載入 AI 提供者、模型與其他偏好；來源分頁不存在或不符合條件時不會誤刷新其他分頁。
- **使用提示模式資訊框版面修正**：桌面版將截圖與代理模式資訊框調整為等寬欄位，並避免 `Screenshot: Off` 標題換行；小尺寸視窗仍維持單欄響應式版面。
- **代理狀態英文翻譯修正**：補齊任務分析、tool calling 不支援、輸出截斷、空回覆與服務不穩定等狀態的英文翻譯，保留等待圖示與執行輪次資訊。
- **自訂提供者名稱顯示修正**：主對話框標題列與提問欄位在有自訂名稱時直接顯示該名稱，未設定時才使用本地化提供者類型名稱；錯誤訊息仍保留完整提供者辨識資訊。
- **多行 LaTeX 公式渲染修正**：在 Markdown 解析前保護完整公式區段，避免 `breaks: true` 將獨立成行的 `$$`、`\[...\]` 與公式環境拆成多個 DOM 節點；同時保留公式內的 `\,` 等 LaTeX 指令，並維持程式碼區塊、一般換行與價格文字的既有行為。
- **宿主網頁 i18n 隔離修正**：停止內容腳本自動掃描宿主 `document`，只對明確登記的 AskPage 文件或 Shadow DOM 套用本地化，避免改寫宿主網站文字、無障礙屬性及 `html` 的 `lang`、`dir`，並消除宿主訊息鍵造成的缺少翻譯警告。

## [0.41.3] - 2026-08-04

### 修正 / 更新（v0.41.3）

- **英文介面 Code fence 空白與換行修正**：修正即時翻譯在處理純空白文字節點時，因前置與後置空白重疊而重複加倍的問題；HTML 與其他程式碼預覽會依原始內容保留正確換行、縮排與空白，收合及展開高度不再被異常內容撐大。
- **程式碼內容翻譯隔離**：英文介面的翻譯流程不再進入 `pre`、`code`、`script` 與 `style` 子樹，避免語法上色建立的動態節點或程式碼中的正體中文文字遭翻譯改寫；Code fence 外的一般介面文字與屬性翻譯維持不變。
- **i18n Code block 回歸測試**：新增純空白冪等性、一般介面翻譯、初始程式碼區塊與動態語法上色節點的測試覆蓋，防止空白倍增或程式碼內容翻譯問題再次發生。

## [0.41.2] - 2026-08-04

### 修正 / 更新（v0.41.2）

- **Azure OpenAI Deployment Name 模型辨識修正**：改以 OpenAI 推理能力表中的最長模型前綴辨識 Azure 部署，讓 `gpt-5.6-luna` 與帶有部署後綴的名稱顯示對應的推理強度控制，並使用該模型的選項與 `medium` 預設值。
- **Azure 推理請求相容性同步**：模型辨識成功後，Azure OpenAI 請求會依使用者選取的推理強度送出對應的 `reasoning.effort` 或 `reasoning_effort`，未知前綴仍不會自動推定。
- **推理能力文件與測試更新**：補充 Azure 前綴辨識規則、最長前綴優先與後綴部署案例，並加入模型專屬選項、日期後綴及未知前綴的測試覆蓋。

## [0.41.1] - 2026-08-03

### 修正 / 更新（v0.41.1）

- **移除重複的頁面上下文整理狀態訊息**：詢問前不再將低資訊量且非即時的頁面上下文整理狀態永久寫入對話紀錄；頁面上下文、選取文字、圖片與截圖的實際處理及 API 請求行為維持不變。

## [0.41.0] - 2026-08-03

### 新增 / 改進（v0.41.0）

- **詢問模式預設串流回應**：對已確認支援串流的 Gemini、OpenAI、Azure OpenAI、Anthropic、DeepSeek、OpenRouter、Groq、Mistral 與 Ollama 模型，詢問模式現在會直接使用對應的串流 API，即時顯示回答與可取得的 reasoning / thinking 內容。
- **Provider 串流能力判定**：新增集中式串流能力表；任意 OpenAI Compatible 端點與未確認的 DeepSeek 模型不會被推定支援串流，會保留一般 JSON 回應以避免送出不相容的 `stream` 請求。
- **串流回應文件與測試**：新增各 Provider 串流端點、模型範圍、SSE 解析流程、能力限制與除錯方式的文件，並補上串流能力判定與詢問模式請求分支測試。
- **Azure 推理能力直接比對模型名稱**：Azure OpenAI 改以 Deployment Name 直接比對能力表中的完整模型 ID；自訂部署或含日期後綴的名稱不會自動套用推理強度，避免誤送不相容的 reasoning 參數。

### 修正 / 更新（v0.41.0）

- **詢問模式串流分支修正**：將串流傳輸與代理模式工具呼叫分離，代理模式仍控制頁面上下文與工具使用，詢問模式則可獨立使用已確認的串流傳輸。
- **Azure 未知部署推理參數防護**：未知或不完全相同的 Azure Deployment Name 不再沿用一般 OpenAI 模型的推理能力判定與預設參數。
...
- **LaTeX 公式預覽**：在 Markdown 回覆渲染流程整合本機 KaTeX 0.18.1，支援 `$$...$$`、`\(...\)`、`\[...\]` 與常見公式環境；公式字型與執行資源皆隨擴充功能封裝，不依賴遠端程式碼。
- **LaTeX 渲染安全性與相容性**：在 Markdown 解析前保護公式分隔符，於 DOMPurify 清理後還原並執行 KaTeX；停用不受信任指令、保留程式碼區塊原文，並避免一般價格文字中的美元符號被誤判為公式。
- **Markdown 粗體標記正規化**：在 Markdown 解析前逐行偵測成對的 `**` 標記，解決粗體緊鄰中文字元或包含全形、半形標點時無法正確渲染的問題。
- **粗體轉換邊界保護**：只有同一行存在偶數個有效標記時才依序轉換為 `<strong>` 與 `</strong>`；單一或奇數個標記維持原文，並排除行內程式碼、跳脫標記、粗斜體與圍欄程式碼區塊。

## [0.37.0] - 2026-07-27

### 新增 / 改進（v0.37.0）

- **新增英文介面**：加入 `zh_TW` 與 `en` 語系資源，並將擴充功能名稱、描述、快捷鍵說明、偏好設定、彈出視窗、CodePen 跳轉頁及頁面對話框等介面納入國際化。
- **依介面語言調整預設回覆**：英文介面送出請求時預設要求模型以英文回答，正體中文介面則維持以 `zh-tw` 回答；英文介面的詢問模式統一顯示為 `Normal`。
- **改善提供者切換操作**：移除對話框標題列原有的獨立提供者按鈕，改由目前模型名稱直接切換 AI 提供者與模型，並補上鍵盤焦點及互動樣式。
- **新增雙語商店上架文件**：補充 Chrome Web Store 的正體中文與英文商店文案、單一用途、權限理由、資料使用揭露、隱私權政策與送審素材需求。

## [0.36.0] - 2026-07-25

### 新增 / 改進（v0.36.0）

- **自訂命令模式增加「不指定」選項**：在偏好設定新增「不指定」模式，新增與編輯自訂命令時可選擇「詢問」、「代理」、「不指定」三個模式。
- **不指定模式保留使用者目前模式**：自訂命令若指定「不指定」，執行時不會切換目前的詢問 / 代理狀態，並保留使用者當前模式；僅對既有截圖設定繼續套用。
- **不指定模式預設置前**：將自訂命令模式預設值調整為「不指定」，並將其顯示順序提升為第一順位，減少建立命令時不小心改變模式的機率。
- **斜線命令支援變數填空**：自訂命令與 `/summary` 提示內容可使用 `${name}` 或 `${name:預設值}` 定義填空；執行時以 hint 標示變數，Tab 可切換不同變數，同名變數會同步修改，變數名稱會以 Unicode NFC 正規化，並支援預設值、復原重做與多行編輯。
- **變數填空顯示方式可切換**：編輯自訂命令或 `/summary` 提示時新增「展開提示詞時顯示變數名稱」勾選框，預設不勾選，未填寫的變數只以變數名稱作為暫時提示顯示、輸入後即消失；勾選後維持「變數名稱：」標籤恆常顯示的展開方式，此設定會隨命令一併儲存。
...
- **修正 Gemini 代理模式工具呼叫組合**：修正 Gemini 3 在代理模式啟用 Google Search 與頁面函式工具同時使用時，必須啟用伺服器端工具呼叫紀錄 `includeServerSideToolInvocations`，否則會發生 API 400 錯誤。
- **修正串流回覆內容保留**：Gemini 代理模式保留內建工具呼叫、回應與 thought signature 等非文字內容，避免下一輪請求失去工具執行上下文。
- **錯誤訊息顯示提供者名稱**：錯誤訊息現在會同時顯示提供者類型與自訂名稱，便於辨識多個同類型提供者；同時避免 `Gemini (Google Gemini)` 這類重複命名。
...
- **重置設定行為調整**：重置時不再保留 AI 提供者、目前模型、加密金鑰等關鍵狀態，而是清除 `chrome.storage.local` 全部資料，避免殘留舊設定造成混淆。
- **重置按鈕文案更新**：將設定頁中的「重置」改為「重置設定」，明確提醒會清除 API Key、斜線命令、提問歷史與其他使用設定。
